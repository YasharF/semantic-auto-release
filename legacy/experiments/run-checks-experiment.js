#!/usr/bin/env node
/**
 * PR Checks Experiment Runner (archived)
 * Steps:
 ...
 * Writes report to experiments/EXPERIMENT_PR_CHECKS.md
 */
const fs = require("fs");
...
async function createBranch(base, branch) {
  ...
  fs.appendFileSync(
    "experiments/CHECKS_EXPERIMENT.md",
    `\nExperiment touch ${new Date().toISOString()}\n`,
  );
  try {
    execSync("npx prettier --write experiments/CHECKS_EXPERIMENT.md", {
      stdio: "inherit",
    });
  } catch (e) {
    console.warn("Prettier format (initial touch) failed:", e.message);
  }
  execSync("git add experiments/CHECKS_EXPERIMENT.md", { stdio: "inherit" });
...
async function run() {
  ...
  fs.writeFileSync(
    "experiments/EXPERIMENT_PR_CHECKS.md",
    reportLines.join("\n"),
  );
  console.log("Report written to experiments/EXPERIMENT_PR_CHECKS.md");
  try {
    execSync("git add experiments/EXPERIMENT_PR_CHECKS.md", {
      stdio: "inherit",
    });
    try {
      execSync("npx prettier --write experiments/EXPERIMENT_PR_CHECKS.md", {
        stdio: "inherit",
      });
    } catch (e) {
