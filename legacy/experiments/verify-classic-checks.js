#!/usr/bin/env node
/**
 * verify-classic-checks.js (archived)
 * Orchestrates:
 * 1. Apply classic branch protection with required contexts fast-check & slow-check (removing rulesets).
 * 2. Run the PR checks experiment script to generate a timeline.
 * 3. Append a Classic vs Rules verification note to experiments/EXPERIMENT_PR_CHECKS.md.
 */
...
async function main() {
  ...
  let experimentJson = "";
  try {
    experimentJson = runCapture("node experiments/run-checks-experiment.js");
  } catch (e) {
  ...
  const reportPath = "experiments/EXPERIMENT_PR_CHECKS.md";
  if (fs.existsSync(reportPath)) {
    ...
    try {
      run("npx prettier --write experiments/EXPERIMENT_PR_CHECKS.md");
    } catch (_) {}
    run("git add experiments/EXPERIMENT_PR_CHECKS.md");
    run(
...
// Archived copy of verify-classic-checks.js
module.exports = require("../scripts/verify-classic-checks");
